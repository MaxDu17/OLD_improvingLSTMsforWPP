...
with tf.Session() as sess:
    ckpt = tf.train.get_checkpoint_state(os.path.dirname('2012/v7/models/'))
    ...
    log_loss = open("2012/v7/GRAPHS/LOSS.csv", "w")
    validation = open("2012/v7/GRAPHS/VALIDATION.csv", "w")
    test = open("2012/v7/GRAPHS/TEST.csv", "w")
    ...
    tf.train.write_graph(sess.graph_def, '2012/v7/GRAPHS/', 'graph.pbtxt')
    writer = tf.summary.FileWriter("2012/v7/GRAPHS/", sess.graph)
    ...
    for epoch in range(hyp.EPOCHS):
        ...
        if epoch%2000 == 0 and epoch>498:
            saver.save(sess, "2012/v7/models/LSTMv7", global_step=epoch)
            print("saved model")
