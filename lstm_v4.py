...
Version 4 changes:
-trying to get rid of output weight and bias for better adaptibility
...
with tf.name_scope("output_gate"): #output gate values to hidden
    ...
    raw_output = tf.reduce_sum(current_hidden, name = "reduce_sum_of_hidden")
...
with tf.Session() as sess:
    ckpt = tf.train.get_checkpoint_state(os.path.dirname('v4/models/'))
    print(ckpt)
    if ckpt and ckpt.model_checkpoint_path:
        saver.restore(sess, ckpt.model_checkpoint_path)

    sm.create_training_set()
    log_loss = open("v4/GRAPHS/LOSS.csv", "w")
    validation = open("v4/GRAPHS/VALIDATION.csv", "w")
    test = open("v4/GRAPHS/TEST.csv", "w")
    ...
    tf.train.write_graph(sess.graph_def, 'v4/GRAPHS/', 'graph.pbtxt')
    writer = tf.summary.FileWriter("v4/GRAPHS/", sess.graph)
    ...
    for epoch in range(hyp.EPOCHS):
        ...
        if epoch%10 == 0:
            ...
            print("The rms loss for this sample is ", loss_)
            print("predicted number: ", output_, ", real number: ", label)

        if epoch%1000 == 0 and epoch>498:
            saver.save(sess, "v4/models/LSTMv4", global_step=epoch)
            print("saved model")
    ...
    print("test: rms is ", RMS_loss)
    test_logger.writerow(["final rms average", RMS_loss])