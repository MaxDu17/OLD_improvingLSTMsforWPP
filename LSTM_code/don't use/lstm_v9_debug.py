...
with tf.name_scope("weights_and_biases"):
    W_Forget = tf.Variable(tf.random_normal(shape = [hyp.hidden_dim + 6,hyp.cell_dim], mean = 10, stddev = hyp.STD, seed = hyp.SEED), name = "forget_weight")
    W_Output = tf.Variable(tf.random_normal(shape=[hyp.hidden_dim + 6,hyp.cell_dim], mean = 10, stddev = hyp.STD, seed = hyp.SEED), name="output_weight")
    W_Gate = tf.Variable(tf.random_normal(shape=[hyp.hidden_dim + 6, hyp.cell_dim],mean = 10, stddev = hyp.STD, seed = hyp.SEED), name="gate_weight")
    W_Input = tf.Variable(tf.random_normal(shape=[hyp.hidden_dim + 6, hyp.cell_dim],mean = 10, stddev = hyp.STD, seed = hyp.SEED), name="input_weight")
...
with tf.name_scope("optimizer"):
    optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.5).minimize(loss) #tempoary thing
